...
def extract_matching_candidates(par_file_path, pulscan_output_path):
    """
    ...
    time = 591.36802816 # Integration time in seconds
    ...
    for file in matching_files:
        try:
            ...
            for i, freq in enumerate(freq_achieve):
                if 0.9999 * freq_expect <= freq <= 1.0001 * freq_expect:
...
def plot_candidate_result(par_file_path, DMs, sigmas, ax):
    ...
    if result is not False:
        _, sigma = result
        positive_candidates.append(os.path.basename(par_file_path))
        ax.scatter(A1, M2, sigma, color='blue', marker='o', label='Pulsar Candidate')
    else:
        negative_candidates.append(os.path.basename(par_file_path))
...
def main():
    ...
    """
    par_file_pattern = '/lustre_archive/spotlight/Nishant/Par_files/Simulation*.par'
    ...
    for par_file in par_files:
        ...
        DMs, sigmas = extract_matching_candidates(par_file, pulscan_output_path)
        positive, negative = plot_candidate_result(par_file, DMs, sigmas, ax)
        positive_all.extend(positive)
        negative_all.extend(negative)

    print("Positive candidates are: ", positive_all)
    print("Negative candidates are: ", negative_all)
